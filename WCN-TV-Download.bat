...
title WCN-TV Downloader

rem ===========================================================
rem   WCN-TV-Download.bat
rem   Archive.org series downloader
rem   - Resume-safe (curl -C -)
rem   - Live progress bar per file
rem   - Fetches the current series list from archive.org
...
set "MAXRETRY=3"
set "LOGFILE=%DESTROOT%\WCN-TV-Download.log"

cls
echo ===========================================================
echo   WCN-TV DOWNLOADER
echo ===========================================================
echo.
echo   Downloads root : %DESTROOT%
echo   Archive item   : %IA_ID%
echo.
echo   - Fetches the current list of series ^(zip files^) from
...
if errorlevel 1 (
    ...
    echo read the series list from archive.org and should be
...
rem ===========================================================
set "MANIFEST=%TEMP%\wcn_tv_manifest.xml"
set "LISTOUT=%TEMP%\wcn_tv_list.txt"
set "TMPPS=%TEMP%\wcn_tv_parse.ps1"

echo.
echo Fetching series list from archive.org ...
...
>>"%TMPPS%" echo $entries = $xml.files.file ^| Where-Object { $_.name -like '*.zip' } ^| Sort-Object { $_.name }
...
if not exist "%LISTOUT%" (
    echo.
    echo ERROR: could not parse the series list.
    echo.
    pause
    goto :eof
)
...
if !COUNT! EQU 0 (
    echo.
    echo No .zip files were found in this archive item yet.
    echo.
    pause
    goto :eof
)
...
cls
echo ===========================================================
echo   WCN-TV DOWNLOADER  -  !COUNT! series available
echo ===========================================================
echo.
...
set "URL=https://archive.org/download/%IA_ID%/!FNAME!"
